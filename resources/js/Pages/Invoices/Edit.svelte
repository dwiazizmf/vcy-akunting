<script>
  ...
  import {
    User, Calendar, ShoppingCart, FileText, MapPin, 
    Ship, Anchor, Plus, Trash2, Save, ArrowLeft, Receipt, Loader2
  } from 'lucide-svelte';
  ...
  $: computedTaxDetails = !isTax ? [] : selectedTaxIds.map(id => {
      const tax = activeTaxes.find(t => t.id === id);
      return { name: tax.name, rate: tax.rate, amount: subtotal * (tax.rate / 100) }; 
  });
...
<AppLayout>
  <div class="max-w-6xl mx-auto space-y-6">
    <div class="flex items-center gap-4">
      <Button variant="outline" size="icon" class="h-8 w-8" on:click={() => window.history.back()}>
        <ArrowLeft class="h-4 w-4" />
      </Button>
      <h1 class="text-2xl font-extrabold tracking-tight text-slate-900">Create New Invoice</h1>
    </div>
    ...
    <Card.Root class="bg-white border-slate-200 shadow-sm">
      <Card.Content class="p-6">
          ...
          <div class="space-y-4 col-span-2 grid grid-cols-2 gap-4">
            ...
            <div class="space-y-1.5">
              ...
              <div class="relative">
                ...
                <Input type="text" bind:value={$form.order_number} placeholder="Order No" class="pl-9 h-9 text-sm bg-white" />
              </div>
            </div>
            <div class="space-y-1.5">
              ...
              <div class="relative">
                ...
                <Input type="text" bind:value={$form.nama_kapal} placeholder="Kapal" class="pl-9 h-9 text-sm bg-white" />
              </div>
            </div>
            ...
            <div class="space-y-1.5">
              ...
              <div class="relative">
                ...
                <Input type="date" bind:value={$form.departure_date} class="pl-9 h-9 text-sm bg-white" />
              </div>
            </div>
            <div class="space-y-1.5">
        ...
        <div class="space-y-4 mb-8">
          ...
          <div class="w-full overflow-x-auto border border-slate-200 rounded-md">
                ...
                {#each $form.items as item, i (i)}
                  <Table.Row class="hover:bg-slate-50 transition-colors">
                    ...
                    <Table.Cell class="p-1.5 text-center">
                      <Button variant="ghost" size="icon" class="h-7 w-7 text-red-500 hover:text-red-700 hover:bg-red-50" on:click={() => removeItem(i)} disabled={$form.items.length === 1}>
            ...
            <div class="p-2 bg-slate-50 border-t border-slate-200">
              <Button variant="outline" size="sm" class="text-xs h-8 text-teal-700 border-teal-200 hover:bg-teal-50" on:click={addItem}>
        ...
        <!-- SUMMARY PANEL -->
        <div class="flex justify-between items-start pt-6 border-t border-slate-100">
          
          <!-- Tax Controls -->
          <div class="w-1/2 bg-slate-50 p-4 rounded-md border border-slate-200">
            <label class="flex items-center gap-2 cursor-pointer w-fit mb-3">
              <input type="checkbox" class="rounded border-slate-300 text-teal-600 focus:ring-teal-500 h-4 w-4" bind:checked={isTax} />
            ...
            {#if isTax}
              <div class="space-y-2 mt-2 pt-2 border-t border-slate-200">
                ...
                {#each activeTaxes as tax}
                  <label class="flex items-center gap-2 cursor-pointer text-sm text-slate-700 hover:bg-slate-100 p-1.5 rounded transition-colors">
                    <input type="checkbox" 
                           class="rounded border-slate-300 text-teal-600 focus:ring-teal-500 h-3.5 w-3.5"
                           checked={selectedTaxIds.includes(tax.id)}
                           on:change={() => toggleTax(tax.id)} />
                    {tax.name} <span class="text-xs text-slate-400">({tax.rate}%)</span>
                  </label>
                {/each}
          ...
          <div class="w-1/3 space-y-3">
            <div class="flex justify-between items-center text-sm">
              ...
              <span class="font-semibold text-slate-700">Rp {subtotal.toLocaleString('id-ID')}</span>
            </div>
            ...
            <div class="flex justify-between items-center text-lg border-t-2 border-slate-800 pt-3 mt-3">
            ...
            <div class="pt-6">
              <Button class="w-full bg-teal-700 hover:bg-teal-800 text-white shadow-sm font-semibold flex items-center justify-center gap-2" on:click={submit} disabled={isProcessing}>
                {#if isProcessing}
