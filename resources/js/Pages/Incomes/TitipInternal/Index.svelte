  ...
  <div class="flex flex-col gap-4 mb-6">
    <div>
      ...
      <p class="text-sm text-muted-foreground mt-1 ml-11.5">
        Kelola dan lacak dokumen titip internal perusahaan
      </p>
    </div>
  </div>
      ...
      <Table.Root>
        <Table.Header class="bg-slate-50/40">
          <Table.Row class="hover:bg-transparent border-slate-150">
            ...
            <Table.Head class="font-bold text-teal-700 text-[11px] uppercase tracking-wider py-3 cursor-pointer hover:text-teal-800 w-28">
        ...
        <Table.Body>
          {#each items as r (r.id)}
            ...
            <Table.Row class={cn(
              ...
              <!-- Up Person -->
              <Table.Cell class="py-3 text-xs text-slate-500 whitespace-normal break-words">
                {r.up_person || '-'}
              </Table.Cell>
              ...
              <Table.Cell class="py-3 text-right pr-5">
                <DropdownMenu.Root>
                  ...
                  <DropdownMenu.Content align="end" class="w-40 bg-white shadow-lg border-slate-100">
                    <DropdownMenu.Item class="text-xs gap-2 cursor-pointer" on:click={() => router.visit(`/titip-internal/${r.id}`)}>
                      <Eye class="h-3.5 w-3.5 text-slate-400" />
                      View Detail
                    </DropdownMenu.Item>
                    <DropdownMenu.Item class="text-xs gap-2 cursor-pointer" on:click={() => router.visit(`/titip-internal/${r.id}/edit`)}>
