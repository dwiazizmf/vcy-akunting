<script>
  ...
  import {
    ...
    ChevronDown, ChevronRight, FileText, ChevronLeft, ChevronRight as ChevronRightIcon,
  ...
  // Toggle selection for all letters
  function toggleSelectAll() {
    const ids = letters.map(r => r.id);
  ...
  function printReport() {
    alert(`Mencetak Surat Tagihan untuk ${selectedRows.length} item terpilih.`);
  }
  ...
  // Helper function to calculate total order numbers dynamically
  function countTotalOrders(letter) {
    if (!letter.invoices) return 0;
    return letter.invoices.reduce((sum, inv) => sum + (inv.orders ? inv.orders.length : 0), 0);
  }
</script>
  ...
  <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
    <div>
      ...
      <p class="text-sm text-muted-foreground mt-1 ml-11.5">
        Kelola berkas surat tagihan dengan master-detail expandable row
      </p>
    </div>
      ...
      <Table.Root>
        <Table.Header class="bg-slate-50/40">
          <Table.Row class="hover:bg-transparent border-slate-150">
            ...
            <Table.Head class="w-12 text-center py-3">
              <input
                type="checkbox"
                checked={letters.length > 0 && letters.map(r => r.id).every(id => selectedRows.includes(id))}
            ...
            <Table.Head class="font-bold text-slate-700 text-[11px] uppercase tracking-wider py-3 w-36">Up</Table.Head>
            <Table.Head class="font-bold text-slate-700 text-[11px] uppercase tracking-wider py-3 w-36">No Tlp</Table.Head>
            <Table.Head class="font-bold text-slate-700 text-[11px] uppercase tracking-wider py-3">Address</Table.Head>
        ...
        <Table.Body>
          {#each letters as letter (letter.id)}
            {@const isExpanded = expandedRows.includes(letter.id)}
            {@const totalOrders = countTotalOrders(letter)}
            <!-- Main Row -->
            <Table.Row class={cn(
              ...
              <Table.Cell class="py-2.5 text-center">
                <button
                  type="button"
                  on:click={() => toggleRowExpansion(letter.id)}
              ...
              <Table.Cell class="py-2.5 text-center">
                <input
                  ...
                  on:change={() => toggleSelectRow(letter.id)}
                  class="rounded border-slate-300 text-teal-600 focus:ring-teal-500 cursor-pointer h-4 w-4"
                />
              </Table.Cell>
              ...
              <!-- Nomor -->
              <Table.Cell class="py-2.5 text-xs text-slate-800 font-semibold font-mono">
                {letter.nomor}
              </Table.Cell>
              ...
              <!-- No Tlp -->
              <Table.Cell class="py-2.5 text-xs text-slate-600 whitespace-nowrap">
                {letter.no_tlp || '-'}
              </Table.Cell>

              <!-- Address -->
              <Table.Cell class="py-2.5 text-xs text-slate-500 max-w-xs truncate" title={letter.address}>
                {letter.address || '-'}
              </Table.Cell>
              ...
              <Table.Cell class="py-2.5 text-right pr-3">
                <DropdownMenu.Root>
                  ...
                  <DropdownMenu.Content align="end" class="w-40 bg-white shadow-lg border-slate-100">
                    <DropdownMenu.Item class="text-xs gap-2 cursor-pointer" on:click={() => router.visit(`/surat-tagihan/${letter.id}`)}>
                      <Eye class="h-3.5 w-3.5 text-slate-400" />
                      View Detail
                    </DropdownMenu.Item>
                    <DropdownMenu.Item class="text-xs gap-2 cursor-pointer" on:click={() => router.visit(`/surat-tagihan/${letter.id}/edit`)}>
                  ...
                  <div class="bg-white border border-slate-150 rounded-xl p-5 shadow-sm space-y-4">
                    ...
                    <div class="flex items-center justify-between border-b border-slate-100 pb-2">
                      <h4 class="text-xs font-bold text-teal-800 uppercase tracking-wider flex items-center gap-1.5">
                        ...
                        Detail Relasi Invoice & Order ({letter.invoices ? letter.invoices.length : 0} Invoices, {totalOrders} Orders)
                      </h4>
                    </div>
                    ...
                    <div class="border border-slate-150 rounded-lg overflow-hidden bg-slate-50/30">
                      <table class="w-full text-xs">
                        ...
                        <tbody class="divide-y divide-slate-150 bg-white">
                          {#each letter.invoices as inv, idx}
