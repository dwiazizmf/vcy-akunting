<script>
  ...
  import {
    Search, Filter, Download, Plus, MoreHorizontal, Eye, Pencil, Trash2,
  ...
  export let filters = { no_kode: '', tanggal_kirim: '', per_page: 25 };

  // ================================================
  // LOCAL STATE
  // ================================================
  let noKode = filters.no_kode || '';
  ...
  function createReport() {
    alert('Membuka form pembuatan Report Mayora baru.');
  }
  ...
  $: hasActiveFilter = noKode || tanggalKirim;
</script>
  ...
  <div class="flex flex-col gap-4 mb-6">
    <div>
      ...
      <p class="text-sm text-muted-foreground mt-1 ml-11.5">
        Kelola dan unduh berkas laporan Mayora
      </p>
    </div>
        ...
        <Table.Body>
          {#each reports as r, idx (r.id)}
            <Table.Row class="transition-colors duration-100 border-slate-100 hover:bg-slate-50/30 align-middle">
              ...
              <!-- Invoice Number -->
              <Table.Cell class="py-3 text-xs text-teal-700 font-semibold font-mono">
                {r.invoice_number}
              </Table.Cell>

              <!-- Order Number -->
              <Table.Cell class="py-3 text-xs text-slate-755 font-mono">
                {r.order_number}
              </Table.Cell>
              ...
              <Table.Cell class="py-3 text-right pr-5">
                <DropdownMenu.Root>
                  ...
                  <DropdownMenu.Content align="end" class="w-40 bg-white shadow-lg border-slate-100">
                    <DropdownMenu.Item class="text-xs gap-2 cursor-pointer" on:click={() => router.visit(`/report-mayora/${r.id}`)}>
                      <Eye class="h-3.5 w-3.5 text-slate-400" />
                      View Detail
                    </DropdownMenu.Item>
                    <DropdownMenu.Item class="text-xs gap-2 cursor-pointer" on:click={() => router.visit(`/report-mayora/${r.id}/edit`)}>
