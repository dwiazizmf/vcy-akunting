<script>
    ...
    export let isEdit = false; // Mode flag
    export let invoice = null; // The invoice to edit

    export let activeTaxes = []; // Supplied by controller
    export let activeDiscounts = []; // Supplied by controller
    ...
    let form = useForm({
        ...
        voy: invoice?.voy || "",
        kode_pelabuhan_asal: invoice?.kode_pelabuhan_asal || "",
        pelabuhan_asal: invoice?.pelabuhan_asal || "",
        kode_pelabuhan_tujuan: invoice?.kode_pelabuhan_tujuan || "",
        ...
        alamat_faktur_pajak: invoice?.alamat_faktur_pajak || "",
    ...
    if (isEdit && invoice) {
        // Map taxes
        let rawTaxes = invoice.header_tax_details;
        if (typeof rawTaxes === 'string') {
            try { rawTaxes = JSON.parse(rawTaxes); } catch (e) { rawTaxes = []; }
        }
        ...
        // Map discounts
        let rawDiscounts = invoice.header_discount_details;
        if (typeof rawDiscounts === 'string') {
            try { rawDiscounts = JSON.parse(rawDiscounts); } catch (e) { rawDiscounts = []; }
        }
        if (Array.isArray(rawDiscounts)) {
            rawDiscounts.forEach(hd => {
